...
#### Création d'une méthode pour récupérer l'utilisateur par l'email ou par le username
...
L'objectif de cette fonction `findUserByEmailOrUsername()` est de renvoyer un utilisateur ou null
- `where()` : on cherche un utilisateur ou l'email soit égal à l'indentifier
- `orWhere()` : ou on veut que le username soit égal à l'identifier
- `setParameter` : Le paramètre identifier correspond à ce qui est passé en paramètre `$usernameOrEmail`
- `setMaxResults(1)` : on s'attend à avoir qu'un seul résultat.
- `getQuery()` : génère la requête
- `getSingleResult()` : est de récupérer résultat seul.
...
En second paramètre le loader est une fonction qui va prendre en paramètre `$identifier` et dans cette fonction on va utiliser le `$userRepository`.
On va utiliser les fonctions fléchés :
Il faut que utilises mon `userRepository` et que tu utilises la méthode `findUserByEmailOrUsername` et en paramètre l'`$identifier`.
...
### Autre manière de gérer l'authentification

La 1ere méthode était d'utiliser `$this->denyAccessUnlessGranted('ROLE_USER')`.
...
On peut utiliser l'attribut PHP `#[IsGranted()]` qui est fourni par le composant `Security`.
Dans cet attribut on spécifiera les rôles qu'il est nécessaire d'avoir pour pouvoir se connecter.
On spécifiera `'ROLE_USER'` donc tout utilisateur n'ayant pas le bon rôle n'y aura pas accès.

Les Rôles :
- `ROLE_USER` : un rôle attribué aux utilisateurs. Il est généralement utilisé pour contrôler l'accès aux fonctionnalités de base de votre application
- `ROLE_ADMIN` : un rôle attribué aux administrateurs. Il est généralement utilisé pour contrôler l'accès aux fonctionnalités d’administration de votre application.

Les expressions de contrôle d'accès :
- `IS_AUTHENTICATED` : c'est une expression qui retourne `true` si l'utilisateur est authentifié, soit par un "remember-me" cookie, soit parce qu’il est pleinement authentifié.
- `IS_AUTHENTICATED_FULLY` : c'est une expression qui retourne `true` si l'utilisateur est pleinement authentifié, c’est-à-dire qu'il a fourni ses identifiants lors de cette session. (à utiliser lorsqu'on a un système de confirmation)
- `IS_REMEMBERED` : c'est une expression qui retourne `true` si l'utilisateur est authentifié via un cookie "remember-me". C’est différent de IS_AUTHENTICATED_REMEMBERED.
- `IS_AUTHENTICATED_REMEMBERED` : c'est une expression qui retourne `true` si l'utilisateur est authentifié, soit pleinement, soit via un cookie "remember-me".
